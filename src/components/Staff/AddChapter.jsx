...
const AddChapter = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      formData.append('volume', chapterData.volume);
      formData.append('mangaid', chapterData.mangaid);

      chapterData.pages.forEach((page) => {
        formData.append('contentImgs', page.file);
      });

      await apiAuth.post(`/api/staff/chapter/manga`, formData, {
        headers: {
    ...
    <div >
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow-lg p-6 mb-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Manga title</label>
            <input
              type="text"
              name="title"
        ...
        <div className="flex justify-end gap-3 pt-4 border-t">
          ...
          <button
            ...
            className={`bg-teal-600 hover:bg-teal-700 text-white px-4 py-2 rounded-md flex items-center transition-colors ${isSubmitting || chapterData.pages.length === 0 ? 'opacity-50 cursor-not-allowed' : ''
              }`}
          >
