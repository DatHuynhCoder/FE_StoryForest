      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {books.map((book, index) => (
          <div key={index} className="flex flex-col bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden sm:flex-row hover:bg-gray-100 dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-700">
            <img className="w-full h-48 object-cover sm:w-36 sm:h-full" src={book.picture} alt={book.name} />
            <div className="flex flex-col justify-between p-3 sm:p-4 leading-normal">
              <h4 className="mb-1 sm:mb-2 text-xl sm:text-2xl font-bold tracking-tight dark:text-white">{book.name}</h4>
              <p className="mb-1 sm:mb-3 font-normal text-green-700 text-sm sm:text-base">{book.author}</p>
              <p className="mb-2 sm:mb-3 font-normal text-black text-sm sm:text-base line-clamp-3">{book.description}</p>
